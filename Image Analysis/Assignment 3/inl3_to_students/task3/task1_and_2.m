...
cnn_model = trainSimpleCNN(X_train,Y_train);
...
% % %     
    ...
    % (don't forget: transpose as necessary, both for X and Y)
     ...
     predictions_test_cnn = predictSimpleCNN(cnn_model, X_test);
%    
    % We can now proceed to computing the respective error rates.
     ...
     pred_test_diff_cnn = predictions_test_cnn - Y_test;
     ...
     err_rate_test_cnn = nnz(pred_test_diff_cnn)/nbr_test_examples;
%     
    % Store them in the containers
     err_rates_test(i, 1) = err_rate_test_tree;
     err_rates_test(i, 2) = err_rate_test_svm;
     err_rates_test(i, 3) = err_rate_test_nn;
     err_rates_test(i, 4) = err_rate_test_cnn;
%     
    ...
    % FILL IN CODE SIMILAR TO THE TEST PART ABOVE!
     ...
     predictions_train_cnn = predictSimpleCNN(cnn_model, X_train);
     ...
     pred_train_diff_cnn = predictions_train_cnn - Y_train;
     ...
     err_rate_train_cnn = nnz(pred_train_diff_cnn)/nbr_test_examples;

     err_rates_train(i, 1) = err_rate_train_tree;
     err_rates_train(i, 2) = err_rate_train_svm;
     err_rates_train(i, 3) = err_rate_train_nn;
     err_rates_train(i, 4) = err_rate_train_cnn;
